...
-- Function to get current timestamp in YYMMDD_HHMMSS format
local function get_timestamp()
  local time = os.date("*t")
  return string.format("%02d%02d%02d_%02d%02d%02d", time.year % 100, time.month, time.day, time.hour, time.min, time.sec)
end
...
local function get_volume_name(file_path)
  ...
  return volume and sanitize_name(volume) or "Unknown_Volume"
end
