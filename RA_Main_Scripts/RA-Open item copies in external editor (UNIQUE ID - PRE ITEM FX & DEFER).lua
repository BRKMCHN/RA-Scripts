-- @description RA-Open item copies in external editor (UNIQUE ID - PRE ITEM FX & DEFER)
-- @version 1.1
-- @author RESERVOIR AUDIO / MrBrock with AI

-- Will duplicate active take of selected items.
-- Will render PRE FX via accessor and replace the duplicated take's source with new item
-- (name appended with unique ID).
-- Will sanitize take parameters which get rendered like start offset, playrate & pitch.
-- Will add the original source's filename to item notes to avoid duplicating the tag later on.
-- Will defer with user defined time delay to allow Reaper to catch up.

---------------------------------------
-- User options
---------------------------------------
local TAG_SUFFIX             = "_RX"   -- tag appended to new file stem
...
local OPEN_IN_EXTERNAL_AFTER = true    -- set false if you ever want to disable

-- NEW: main folder where rendered files should be written.
-- Example:
-- local MAIN_SOURCE_DIR = "/Volumes/WorkDrive/MyProject/Sources"
-- If left empty, script falls back to current Reaper project directory.
local MAIN_SOURCE_DIR        = ""
...
-- Helpers
---------------------------------------
local function open_in_external_editor_with_delay(start_t)
  if not OPEN_IN_EXTERNAL_AFTER then return end

  if not start_t then
    start_t = reaper.time_precise()
  end

  local elapsed = reaper.time_precise() - start_t

  if elapsed < 0.20 then
    reaper.defer(function() open_in_external_editor_with_delay(start_t) end)
    return
  end
...
local function get_output_root()
  if MAIN_SOURCE_DIR and MAIN_SOURCE_DIR ~= "" then
...
local function plan_output_channels(chanmode, src_ch)
  ...
  elseif chanmode == 2 then
    out.out_ch = 1
    out.kind = "mono_mix_12"

  elseif chanmode >= 3 and chanmode <= 66 then
    local ch_index = chanmode - 2
    out.out_ch = 1
    out.kind = "mono_pick"
    out.pick = ch_index

  elseif chanmode >= 67 then
    local start = (chanmode - 67) + 1
    out.out_ch = 2
    out.kind = "stereo_pair"
    out.left = start
    out.right = start + 1
...
---------------------------------------
local function snapshot_selected_items(sel_count)
  local snap = {}
  for i = 0, sel_count - 1 do
    local it = reaper.GetSelectedMediaItem(0, i)
    snap[#snap + 1] = {
      item = it,
      takes_before = reaper.CountTakes(it) or 0
    }
  end
  return snap
end
...
---------------------------------------
-- Render duplicated take to file
---------------------------------------
local function render_dup_take_to_file(dup_take)
  ...
  dbg(
    "Take %s | src_ch=%d I_CHANMODE(saved)=%d plan=%s out_ch=%d frames=%d sr_src=%d",
...
if _orig_grouping then
  reaper.Main_OnCommand(CMD_TOGGLE_ITEM_GROUPING, 0)
end

local snap = snapshot_selected_items(sel_count)
...
if orig_move_state == true then
  set_toggle_state(CMD_MOVE_TAKE_ENV_WITH_CONTENTS, false)
end
...
if orig_move_state == true then
  set_toggle_state(CMD_MOVE_TAKE_ENV_WITH_CONTENTS, true)
end
...
reaper.Undo_EndBlock(
  "Dup active take -> Render (RAW @ source SR, sample-aligned) -> Replace duplicate; neutralize timing; RA_OFN naming; no FX",
  -1
)
...
reaper.defer(function()
  open_in_external_editor_with_delay()
end)