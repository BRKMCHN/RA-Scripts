-- @version 1.1
-- @description Sort selected items by length and optimize vertical track usage within user-specified overlap threshold (0–100%).
-- @author MrBrock / AI
-- V1.1: Added user prompt for overlap threshold.
    ...
    if #timeline > 0 then
        ...
        for i = 2, #timeline do
            ...
            if current_interval.finish >= next_interval.start then
                -- Overlapping intervals: merge by extending end
...
function move_item_to_highest_track(item, lowest_track_num, items_to_unselect)
    local highest_track = reaper.GetTrack(0, lowest_track_num - 1) -- get track
    reaper.MoveMediaItemToTrack(item, highest_track)
    table.insert(items_to_unselect, item)
end

-- Step 10: Cycle through each item in the sorted list, using a user-defined overlap_threshold
function process_items(all_items, lowest_track_num, items_to_unselect, overlap_threshold)
    for _, current_item in ipairs(all_items) do
        if current_item.track_num == lowest_track_num then
            -- Item is already on that track, skip
        else
...
function main()
    ...
    -----------------------------------------------------------
    -- (A) Prompt user for overlap percentage (0 to 100)
    -----------------------------------------------------------
    local ret, user_val_str = reaper.GetUserInputs("Overlap Threshold", 1, 
        "Overlap % (0-100):", "50")
    if not ret then 
        return -- user canceled
    end

    local user_val = tonumber(user_val_str)
    if not user_val then
        reaper.ShowMessageBox("Invalid number entered. Aborting.", "Error", 0)
        return
    end

    -- Clamp to 0..100
    if user_val < 0 then user_val = 0 end
    if user_val > 100 then user_val = 100 end

    -- Convert to 0..1
    local overlap_threshold = user_val / 100.0

    -----------------------------------------------------------
    -- (B) Store the currently selected tracks & items
    -----------------------------------------------------------
    local stored_tracks = {}
    ...
    -----------------------------------------------------------
    -- (C) Unselect all tracks
    -----------------------------------------------------------
    unselect_all_tracks()

    -----------------------------------------------------------
    -- (D) Reselect the tracks that contain the selected items,
    ...
    while reaper.CountSelectedMediaItems(0) > 0 do
        ...
        -- Step 10: Process, now passing our user-defined threshold
        process_items(all_items, destination_track_num, items_to_unselect, overlap_threshold)
    ...
    -----------------------------------------------------------
    -- (E) Restore original track selection
    -----------------------------------------------------------
    unselect_all_tracks()
    ...
    -----------------------------------------------------------
    -- (F) Restore original item selection
    -----------------------------------------------------------
